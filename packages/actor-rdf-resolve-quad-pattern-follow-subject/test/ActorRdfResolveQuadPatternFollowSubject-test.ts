...
describe('ActorRdfResolveQuadPatternFollowSubject', () => {
  ...
  describe('An ActorRdfResolveQuadPatternFollowSubject instance', () => {
    ...
    it('should test', () => {
      return expect(actor.test({ pattern: null, context: { sources: [{ type: 'traverselinks', value: true  }] } }))
        .resolves.toBeTruthy();
    });
    ...
    it('should not test on a falsy traverselinks value', () => {
      return expect(actor.test({ pattern: null, context: { sources: [{ type: 'traverselinks', value: null  }] } }))
        .rejects.toBeTruthy();
    });
    ...
    it('should run on a named node subject', () => {
      ...
      return actor.run({ pattern, context: { sources: [{ type: 'traverselinks', value: true }] } })
        .then(async (output) => {
